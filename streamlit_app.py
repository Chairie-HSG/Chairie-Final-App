...
from interactive_map import render_interactive_map, handle_seat_selection, load_map_data
...
def main_app():
    ...
    st.markdown(
        "<div style='font-size:24px; font-weight:bold; margin-bottom:16px; color:#2b6f95;'>"
        "Library Ground Floor - Interactive Map"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        "Click on any seat dot to view details and reserve. "
        "Green = Available, Orange = Reserved, Red = Occupied"
        "</div>",
        unsafe_allow_html=True,
    )
    
    # Render interactive map with seats from JSON
    render_interactive_map(map_data["seats"], st.session_state.get("selected_seat_id"))
    
    # Check for seat selection from URL params
    selected_seat_from_url = handle_seat_selection(seats)
    if selected_seat_from_url:
        st.session_state["selected_seat_id"] = selected_seat_from_url["id"]
    
    # Seat Details Section (anchor target)
    st.markdown(
        "<div id='seat-details-section' style='scroll-margin-top: 100px;'></div>",
        unsafe_allow_html=True,
    )
    ...
    if not selected_id:
        st.info("👆 Click on any seat dot on the map above to see seat details and make a reservation.")
        return
    ...
    st.markdown(
        ...
        <div style="background:#eef5f8; border-radius:10px; padding:16px; font-size:15px;
                   margin-bottom:16px; border-left: 4px solid {seat_status_color(seat['status'])};">
    ...
    if seat["status"] == "free":
        st.success("✅ This seat is free and available for reservation.")
        if st.button("Reserve this seat (10 min)", key=f"reserve_{seat['id']}", type="primary"):
        ...
        if seat["reserved_by_me"]:
            ...
            c1, c2 = st.columns(2)
            with c1:
                if st.button("✓ Check In", key=f"checkin_{seat['id']}", type="primary"):
            ...
            with c2:
                if st.button("Cancel Reservation", key=f"cancel_{seat['id']}"):
                    ...
                    if result["success"]:
                        ...
                        st.error(result["message"])
            st.caption("In the real app, 'Check In' would be replaced by a QR code scan.")
        else:
